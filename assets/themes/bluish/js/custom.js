...
$(document).ready(function(){
    ...
    /* Slim Scroll for the LOGS */
    $('div.logs-list').slimScroll({
        distance: '0px',
        color: '#777',
        height: '300px',
        wheelStep: 1
...
$(window).load(function(){
    $( 'body' ).trigger( 'click' );
    setTimeout(function(){
        $('.containerk').addClass('no-border');
        $('.navbar').addClass('nav-border');
    }, 600);
});
/*Projects Drop down init and code */
$('.project-switcher-btn').click(function(e){
    e.preventDefault();
    $('.pdd').addClass('pdd-visible');
    $(this).parent().toggleClass("open");
    var $container = $('#horiz_container_outer');
    var parentPosition = $container.parent().position();
    var containerHeight = ( parentPosition.top > 1) ? '-200px' : '48px';
    $container.slideToggle();
    $('#scrollbar').removeAttr('style');
    $('#scrollbar').toggleClass('sb-invisible');
});

/* Hide Projects Drop down if other element clicked */
$('body').click(function(event) {
    var $myelem = $('.project-switcher-btn');
    var $container = $('#horiz_container_outer');
    if ((!$(event.target).closest($myelem).length)&&(!$(event.target).closest($container).length)) {
        $myelem.parent().removeClass("open");
        $container.slideUp();
        $('#scrollbar').addClass('sb-invisible');
    };

});

/*Close Sidebar Search block*/
$('.sidebar-search-results i.close').on('click', function(e) {
   e.preventDefault();
   $(this).parent().slideUp();
});
/*Open Sidebar Search block*/
$('.sidebar-search button.submit').on('click', function(e) {
    e.preventDefault();
    var $myelem = $('.sidebar-search-results');
    $myelem.slideDown();
});
